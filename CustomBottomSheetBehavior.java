 ...
 * Much of this code is borrowed from https://github.com/miguelhincapie/CustomBottomSheetBehavior
 * and modified to fit this project.
 */

public class CustomBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, View child, int layoutDirection) {
        ...
        mViewRef = (WeakReference<V>) new WeakReference<>(child);
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, View child, View target) {
        ...
        if (scrollVelocity > mMinimumVelocity) {
            ...
            // Not flinging, just settle to the nearest state
                ...
                int currentTop = child.getTop();
                int drawerAtAnchorHeight = mMaxOffset - mAnchorPoint;

                // Currently at collapsed state
                if (mLastStableState == STATE_COLLAPSED) {
                    // If moved more than 1/3 of distance, snap to anchor
                    if (currentTop < mMaxOffset - (drawerAtAnchorHeight / 3)) {
                        top = mAnchorPoint;
                        targetState = STATE_ANCHOR_POINT;
                    }
                    // Otherwise, snap back to collapsed
                    else {
                        top = mMaxOffset;
                        targetState = STATE_COLLAPSED;
                    }
                ...
                // Currently at expanded state
                else if (mLastStableState == STATE_EXPANDED) {
                    // If moved more than 1/3 of distance, snap to anchor
                    if (currentTop > mAnchorPoint / 3) {
                        top = mAnchorPoint;
                        targetState = STATE_ANCHOR_POINT;
                    }
                ...
                // Currently at anchor state
                else {
                    // If moved more than 1/3 of distance toward expanded, snap to expanded
                    if (currentTop < mAnchorPoint * 2 / 3) {
                        top = mMinOffset;
                        targetState = STATE_EXPANDED;
                    }
                    // If moved more than 1/3 of distance toward collapsed, snap to collapsed
                    else if (currentTop > mAnchorPoint + (drawerAtAnchorHeight / 3)) {
                        top = mMaxOffset;
                        targetState = STATE_COLLAPSED;
                    }
                    // Snap back to anchor state
                    else {
                        top = mAnchorPoint;
                        targetState = STATE_ANCHOR_POINT;
                    }
    ...
    /**
     * A utility function to get the {@link CustomBottomSheetBehavior} associated with the {@code
     * view}.
     *
    ...
    public static <V extends View> CustomBottomSheetBehavior<V> from(V view) {
        ...
        if (!(behavior instanceof CustomBottomSheetBehavior)) {
            throw new IllegalArgumentException(
                    "The view is not associated with BottomSheetDrawerBehavior");
        }
